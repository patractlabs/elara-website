  ...
  useMemo(() => {
    ...
    setChanges((oldChanges) => {
      const nums = splitedNums.map((num, index) => {
        ...
        const oldNumChange =
          oldChanges[oldChanges.length - (splitedNums.length - index)]
        const change = `${
          oldNumChange ? oldNumChange[oldNumChange.length - 1] : ''
        }-${num}`
                    ...
                    ? {
                        ...
                        animationDuration:
                          to > from
                            ...
                            : `${
                                0.8 *
                                (fromDuration / (fromDuration + toDuration))
                              }, ${
                                0.8 * (toDuration / fromDuration + toDuration)
                              }`,
                        // animationDuration: to > from ? `${duration}s` : `${fromDuration}, ${toDuration}`,
                        animationDelay:
                          to > from ? '0s' : `0s, ${fromDuration}`,
                        animationName:
                          to > from
                            ? `countup${change}`
                            : `countup${from}-10, countup0-${to}`,
                      }
                    : {}
                }
              >
